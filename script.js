...
class OrdersManager {
    ...
    closeWhatsAppModal() {
        ...
        // Reset the status dropdown to original value if user cancelled
        if (this.pendingStatusChange) {
            const dropdown = document.querySelector(`[data-order-id="${this.pendingStatusChange.order.id}"] .status-select`);
            if (dropdown) {
                dropdown.value = this.pendingStatusChange.oldStatus;
            }
            this.pendingStatusChange = null;
        }
    }
    ...
    sendWhatsAppNotification() {
        ...
        const message = this.generateWhatsAppMessage(order, newStatus);
